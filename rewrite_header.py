...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-[#051428]/95 backdrop-blur-xl border-t border-cyan-900/30 px-2 py-2 pb-safe flex items-center justify-between overflow-x-auto no-scrollbar gap-1 shadow-[0_-5px_20px_rgba(0,0,0,0.3)]">
        {navItems.map((item) => {
          const Icon = item.icon;
          const isActive = activeTab === item.id;
          return (
