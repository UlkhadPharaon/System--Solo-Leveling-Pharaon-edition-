...
### BUG-001 — Identité d'app triple, cible « Ka Rise » · `bloquant / incohérence` · `open`
...
### BUG-002 — Génération LLM morte : modèle reasoning vs timeout 20 s · `majeur / bug` · `open`
...
### BUG-003 — Coach IA : profil legacy injecté hors contexte Domain · `majeur / incohérence` · `open`
...
### BUG-004 — `flag_for_human_review` jamais implémenté · `majeur / dette` · `open`
...
### BUG-005 — Sync/leaderboard contre un projet Supabase mort · `majeur / bug` · `open`
...
### BUG-006 — Image Unsplash 404 (Radar Donjons) · `mineur / bug` · `open`
...
### BUG-007 — Fichiers morts · `mineur / dette` · `open`
...
### BUG-008 — PRD.md / DATA_MODEL.md fantômes · `mineur / dette` · `open`
...
### DISC-001 — 7 URLs Unsplash codées en dur (Donjons) · `majeur / dette` · `open`
...
### DISC-002 — Quêtes générées jamais affichées (composant orphelin) · `majeur / fonctionnel` · `open`
...
### DISC-003 — Fetch leaderboard inconditionnel au montage du Dashboard · `majeur / perf` · `open`
...
### DISC-004 — fx.ts : fetch taux de change sans timeout · `mineur / perf` · `open`
...
### DISC-007 — Classement sans état d'erreur visible · `mineur / UX` · `open`
