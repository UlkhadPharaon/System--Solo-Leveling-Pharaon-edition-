...
export const PharaohAvatarCustomizer: React.FC<PharaohAvatarCustomizerProps> = ({
  ...
  const currentAura = AURAS.find(a => a.value === customization.auraColor) || AURAS[0];
      ...
      <div className="lg:col-span-5 bg-sl-primary/40 border border-sl-gold/15 rounded-3xl p-6 flex flex-col items-center justify-center relative min-h-[350px] shadow-gold-sm overflow-hidden">
        ...
        <div className="relative w-56 h-56 z-10 flex items-center justify-center">
          <svg viewBox="0 0 100 100" className="w-full h-full drop-shadow-[0_0_15px_rgba(212,175,55,0.15)]">
            ...
            {/* Head Shape */}
            <circle cx="50" cy="50" r="18" fill={customization.skinTone} stroke="url(#goldGrad)" strokeWidth="0.75" />

            {/* Eyes Glow Effect */}
            <circle cx="43" cy="48" r="1.5" fill={customization.eyeColor} filter={`drop-shadow(0 0 2px ${customization.eyeColor})`} />
            <circle cx="57" cy="48" r="1.5" fill={customization.eyeColor} filter={`drop-shadow(0 0 2px ${customization.eyeColor})`} />
            ...
            {customization.crownType === 'pschent' && (
              <g>
            ...
            {customization.crownType === 'khepresh' && (
              <g>
                {/* Khepresh War Crown */}
            ...
            {AURAS.map((a) => (
              <button
                ...
                className={`p-3 rounded-xl border text-left transition-all relative overflow-hidden ${
                  customization.auraColor === a.value
            ...
            {CROWNS.map((c) => (
              <button
                ...
                className={`p-3.5 rounded-xl border text-left transition-all ${
                  customization.crownType === c.value
                    ? 'bg-sl-lapis/50 border-sl-gold text-white shadow-gold-sm'
                    : 'bg-sl-primary/40 border-sl-gold/15 text-slate-400 hover:border-sl-gold/30 hover:text-white'
                }`}
              >
