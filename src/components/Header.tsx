...
interface HeaderProps {
  ...
  isOffline?: boolean;
  /** Hide the workout tab when the user declared no physical domain (onboarding v2). */
  showWorkoutTab?: boolean;
}

// Isolated so the per-second tick only re-renders the clock, not the whole Header/nav.
...
export const Header: React.FC<HeaderProps> = ({
  ...
  const navItems: { id: ActiveTab; label: string; icon: React.FC<{ className?: string }>; highlight?: boolean }[] = [
    ...
    ...(showWorkoutTab ? [{ id: 'workout' as ActiveTab, label: 'Entraînement', icon: Dumbbell }] : []),
  ...
  return (
    <>
          ...
          <div className="flex flex-wrap items-center gap-2 justify-end">
            ...
            <div className="hidden md:flex items-center gap-2 mr-4">
              ...
              {personalization?.hunterTitle && (
                <div className="hidden lg:flex items-center px-3 py-1.5 rounded-lg bg-sl-gold/10 border border-sl-gold/40 text-sl-gold-light font-display text-[10px] tracking-wider anim-in-right">
                  {personalization.hunterTitle}
                </div>
              )}
            </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`btn-press flex items-center gap-2 px-4 py-2 rounded-lg text-sm transition-all whitespace-nowrap ${
                  isActive
                    ? item.highlight
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-[#051428]/95 backdrop-blur-xl border-t border-cyan-900/30 px-2 py-2 pb-[calc(1rem+env(safe-area-inset-bottom,0px))] flex items-center justify-between overflow-x-auto no-scrollbar gap-1 shadow-[0_-5px_20px_rgba(0,0,0,0.3)] safe-bottom">
        {navItems.map((item) => {
          const Icon = item.icon;
          const isActive = activeTab === item.id;
          return (
            <button
              key={item.id}
              onClick={() => setActiveTab(item.id)}
              className={`btn-press flex flex-col items-center justify-center min-w-[60px] p-2 rounded-xl transition-all ${
                isActive
                  ? item.highlight
                    ? 'text-cyan-300'
                    : 'text-white'
